...
class OntologyTripleParserTest {
	...
	void collectTriplesFromFile_containsMacrophageSubClassOfTriples() {
		...
		List<Triple> macrophageSubClassTriples = triples.stream()
				.filter(t -> t.getSubject().getURI().equals(macrophageUri))
				.filter(t -> t.getPredicate().getURI().equals(subClassOfUri))
				.toList();

		assertFalse(macrophageSubClassTriples.isEmpty());

		List<String> objectUris = macrophageSubClassTriples.stream()
	...
	void collectTriplesFromFile_flattenedRestrictions() {
		...
		boolean found = triples.stream().anyMatch(t -> t.getSubject().getURI().equals(macrophageUri)
				&& t.getPredicate().getURI().equals(developsFromUri)
				&& t.getObject().isURI() && t.getObject().getURI().equals(monocyteUri));
	...
	void collectTriplesFromFile_capableOfRestriction() {
		...
		boolean found = triples.stream().anyMatch(t -> t.getSubject().getURI().equals(macrophageUri)
				&& t.getPredicate().getURI().equals(capableOfUri)
				&& t.getObject().isURI() && t.getObject().getURI().equals(pseudopodiumOrgUri));
	...
	void collectTriplesFromFile_containsLiteralTriples() {
		...
		String macrophageUri = "http://purl.obolibrary.org/obo/CL_0000235";
		boolean hasLiteral = triples.stream()
				.filter(t -> t.getSubject().getURI().equals(macrophageUri))
				.anyMatch(t -> t.getObject().isLiteral());
	...
	void collectUniqueTriples_skipsRoOwl() {
		...
		boolean hasRoSubject = uniqueTriples.stream()
				.anyMatch(t -> t.getSubject().isURI() && t.getSubject().getURI().startsWith(roNS));
